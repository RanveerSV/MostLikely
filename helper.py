...
def parse_chat(file_path):
    ...
    df = df[~df['Message'].str.contains('|'.join(noise), case=False)]
    low_signal_reactions = {'ok', 'k', 'yes', 'no', 'lol', 'lmao', 'yo', 'hi', 'hello', 'yep', 'hmm', 'oh'}
    
    def is_meaningful(msg):
        msg_clean = msg.lower().strip()
        words = msg_clean.split()
        # Keep if it has more than 2 words AND is not just a single low-signal reaction
    ...
    # Remove rows where final aggregated message is empty
